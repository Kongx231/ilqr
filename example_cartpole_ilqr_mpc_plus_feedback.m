...
current_state = init_state + [0;-pi/4;-1;1]; % Add larger perturbation
...
for ii=1:duration
    ...
    %     % Comment in to use classic linear feedback (may be unstable)
    %     current_error = current_state - states(ii,:)';
    %     current_feedback_gain = reshape(original_K(ii,:,:),n_inputs,n_states);
    %     current_input = inputs(ii,:)' + current_feedback_gain*current_error;
