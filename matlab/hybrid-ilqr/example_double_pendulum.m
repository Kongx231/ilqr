...
% symbolic_dynamics_double_pendulum_hybrid();
...
n_states = 4;
n_inputs = 2;
init_state = [-pi*3/4-pi/12;pi/2+pi/4;0;0]; % Define the initial state to be the origin with no velocity
init_mode = 1;
target_state = [-pi*3/4;pi/2-pi/24;0;0]; % Swing pendulum upright
...
mass1 = 1;
length1 = 1;
mass2 = 1; % more mass to swing better
length2 = 1;
gravity = 9.8;
...
save('double-pendulum-trajectory.mat','states','inputs','modes','trajectory_struct','dt','parameters','k_feedforward','K_feedback'); % Save trajectory to track later
...
figure(3);
animate_double_pendulum(states,dt,modes)
