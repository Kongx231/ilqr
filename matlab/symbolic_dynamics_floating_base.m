...
% hip_pos_1 = robot_pos + hip_offset_1;
% hip_pos_2 = robot_pos + hip_offset_2;
% hip_pos_3 = robot_pos + hip_offset_3;
% hip_pos_4 = robot_pos + hip_offset_4;
...
% Map body to world angular velocities
...
% Get angular acceleration from the wrenches
ang_accel_world = simplify(I_hat_inv*(skew(r_1)*F_1 + skew(r_2)*F_2 + skew(r_3)*F_3 + skew(r_4)*F_4));

% Get linear accleartion from the forces
gravity_vec = [0;0;g];

linear_accel = 1/m*(F_1+F_2+F_3+F_4)+gravity_vec;

% Ordering, position, ang position, linear vel, angular vel
